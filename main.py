    ...
    def __init__(self):
        ...
        try:
            ...
            if not self.base_url:
                raise ValueError(
                    "MAIN_URL_SCRAPE_ROXO environment variable is not set or is empty")
            ...
            if self.root:  # Check if Tkinter root was successfully created before the error
                ...
                messagebox.showerror(
                    "Application Error", f"Failed to initialize the application GUI:\n{e}")
                self.root.destroy()
            else:
                # If root couldn't even be created, log to stderr as a last resort
                sys.stderr.write(
                    f"CRITICAL: Failed to create Tkinter root window: {e}\n")
                sys.stderr.flush()
    ...
    def start_monitoring(self, keywords, telegram_token, chat_input):
        ...
        try:
            ...
            telegram_bot = TelegramBot(
                log_callback=self.gui.log, token=telegram_token)
...
def main():
    try:
        ...
        try:
            ...
            sys.stderr.write(
                f"FATAL: Application encountered an unhandled error: {e}\n")
            sys.stderr.flush()
