...
class MarketRoxoGUI:
    def __init__(self, root, start_callback, stop_callback):
        ...
        load_dotenv(dotenv_path=dotenv_path) # Load environment variables from specified path
        # --- END CHANGE ---

        self.default_keywords = os.getenv("DEFAULT_KEYWORDS","iphone, samsung, xiaomi")
        self.telegram_token = os.getenv("TELEGRAM_TOKEN", "").strip()
        self.cellphone_number = os.getenv("TELEGRAM_CHAT_ID_OR_PHONE", "").strip()
    ...
    def create_widgets(self):
        ...
        self.keywords_entry.insert(0, self.default_keywords)  # Default keywords
        ...
        self.telegram_token_entry.insert(0, self.telegram_token)  # Default token
        ...
        self.start_button = tk.Button(button_frame, text="🚀 Iniciar Monitoramento", 
                                     command=self._on_start, font=("Arial", 12, "bold"),
    ...
    def _on_start(self):
        ...
        try:
            ...
            if not keywords_list:
                messagebox.showerror("Erro", "Digite pelo menos uma palavra-chave válida!")
                return
                
            # Update UI state
            self.start_button.config(state=tk.DISABLED, bg='gray')
            self.stop_button.config(state=tk.NORMAL, bg='#f44336')
            self.status_label.config(text="Status: Iniciando monitoramento...", fg="orange")
    ...
    def _on_stop(self):
        ...
        try:
            self.status_label.config(text="Status: Parando monitoramento...", fg="orange")
            self.stop_callback()
            self._reset_buttons()
            self.status_label.config(text="Status: Monitoramento parado", fg="red")
            
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao parar monitoramento:\n{str(e)}")
    ...
    def set_monitoring_active(self):
        ...
        self.status_label.config(text="Status: Monitoramento ATIVO", fg="green")
