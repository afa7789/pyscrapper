...
class Monitor:
    ...
    def start(self):
        ...
        while self.running:
            try:
                ...
                self.log_callback(f"🔍 Verificação #{cycle_count} - {current_time}")
                
                #  when we add multiple scrapers, we can loop through them
                #  and scrape each adding the results to new_ads
                new_ads = self.scraper.scrape(self.keywords,self.negative_keywords_list,1)
            ...
            # Wait with countdown
            if self.running:
                self.log_callback(f"⏳ Aguardando próxima verificação ({seconds_to_wait/60} minutos)...")
    ...
    def _split_message(self, ads):
        ...
        for i in range(0, len(ads), self.batch_size):
            ...
            message_header = f"🔔 Novos anúncios encontrados (parte {len(messages) + 1} de { (len(ads) + self.batch_size - 1) // self.batch_size }):\n\n"
            message_content = "\n\n".join(batch)
